	...
	FOREIGN KEY (`player_id`) REFERENCES `players` (`id`)\
		...
		case 27:
		{
			...
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
					db->executeQuery(std::string("ALTER TABLE `players` ADD `lookmount` INT(11) NOT NULL DEFAULT '';"));
					db->executeQuery(std::string("ALTER TABLE `players` ADD `currmount` INT(11) NOT NULL DEFAULT '';"));
					break;
				}

				case DATABASE_ENGINE_MYSQL:
				{
					db->executeQuery(std::string("ALTER TABLE `players` ADD `lookmount` INT(11) NOT NULL DEFAULT 0 AFTER `lookaddons`;"));
					db->executeQuery(std::string("ALTER TABLE `players` ADD `currmount` INT(11) NOT NULL DEFAULT 0 AFTER `lookmount`;"));
					break;
				}
