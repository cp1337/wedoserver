...
bool argumentsHandler(StringVec args)
{
        ...
        for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
        {
                ...
                if((*it) == "--version")
                {
                        ...
                        "Compiled with: Team We do.\n"
                        "A server developed by: Team We do & CiA.\n"
                        "Visit our forum for updates & support: http://xtibia.com/.\n";
                        return false;
                }
...
ServiceManager* services)
{
        ...
        std::cout << "Compiled with: Team We do." << std::endl;
        std::cout << "A server developed by: Team We do & CiA." << std::endl;
        std::cout << "Visit our forum for updates & support: http://xtibia.com/." << std::endl << std::endl;
        ...
        mutexName << "wedoserver_" << g_config.getNumber(ConfigManager::WORLD_ID);

        CreateMutex(NULL, FALSE, mutexName.str().c_str());
        if(GetLastError() == ERROR_ALREADY_EXISTS)
                startupErrorMessage("Another instance of We do Server is already running with the same worldId.\nIf you want to run multiple servers, please change the worldId in configuration file.");
                        ...
                        switch(LOWORD(wParam))
                        {
                                ...
                                case ID_MENU_MAIN_CLEARLOG:
                                {
                                        if(g_game.getGameState() != GAME_STATE_STARTUP)
                                        {
                                                ...
                                                std::cout << "Compiled with: Team We do." << std::endl;
                                                std::cout << "A server developed by: Team We do & CiA." << std::endl;
                                                std::cout << "Visit our forum for updates & support: http://xtibia.com/." << std::endl;
                                                std::cout << std::endl;
                                        }
...
int32_t WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int32_t WindowStyle)
{
        ...
        wincl.lpszClassName = "wedoserver_gui";
        ...
        GUI::getInstance()->m_mainWindow = CreateWindowEx(0, "wedoserver_gui", STATUS_SERVER_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 640, 450, HWND_DESKTOP, NULL, hInstance, NULL);
